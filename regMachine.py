...
pw = ""
CRNs = [10121, 10222]
TIMETICKET = datetime(year=2020, month=8, day=24,
...
def login():
    try:
        ...
        searchPassword = driver.find_element_by_xpath(
            "//input[@type='password']")
...
def addCourse():
    ...
    buttom = driver.find_element_by_xpath(
...
def wait():
    ...
    while not correctPeriod:
        now = datetime.now()
        if((now + timedelta(seconds=5)) >= TIMETICKET):
            print("start entering the reg pages!")
            driver.refresh()
